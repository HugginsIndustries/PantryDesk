...
- [ ] Add simple human-friendly codes and ID card generation
  - Impact: High
  - Complexity: Medium
  - Acceptance Criteria:
    ...
    2. **MemberCode:** Add `MemberCode` column to the HouseholdMembers table (for search/tracking). UNIQUE, stable. Format: `"M"` + 5 digits zero-padded (e.g. M00001, M01234). Comma-delimited `HouseholdCode,MemberCode` used when member is known (e.g. search, manual entry). MemberCode and member names are NOT printed on ID cards (privacy—lost cards have no PII). MemberCode may be used for search/manual entry and internal tracking, but is not displayed in UI by default.
    3. **EventType SelfServe:** Add support for service_events.event_type 'SelfServe' (unattended deck-only flow). Include in migration if event_type has constraints; otherwise document as valid value. Ensure SelfServe is handled everywhere event-type stats are used: Statistics Dashboard (Event Type breakdown), Reports, PDF exports, and any other place that uses PantryDay/Appointment counts (e.g. add SelfServeCompletions to models and Event Type pie/table). SelfServe is never included in Monthly Activity Report ‘Days Open’ and does not affect pantry-day counts. SelfServe/Deck Only services never count toward the 1-visit-per-month shop rule.
    4. **Auto-generation on create (counter-based):** Use a DB-backed counter instead of MAX(existing code)+1. Add a `CodeCounters` table (or reuse AppConfig) with keys "HouseholdCodeNext" and "MemberCodeNext" (integers, start at 1). On create, within a single transaction: read current next value, generate code (H{num:00000} or M{num:00000}), increment and persist counter, insert row. Guarantees uniqueness without retry loops and prevents race issues. Never reuse codes—deleted members/households do not decrement or recycle. Enforce uniqueness at DB level (unique index/constraint). **Migration/backfill:** Assign codes sequentially to all existing households/members; then set counters to (max assigned + 1) for each type.
    5. **Display:** Show HouseholdCode as read-only on the Household Profile main tab. MemberCode stays in DB for search/tracking; not displayed in UI.
    6. **ID card layout:** 2" x 3.5" per card (to fit lamination pouches). Content: "Winlock-Vader Food Bank ID"; Household ID: {HouseholdCode}; barcode (Code 128, HouseholdCode only—no member info, no PII). Black border around each card; margin around each for cutting; evenly spaced on page with page margins for printing. Up to 8 cards per letter-size PDF. Number-of-cards selector (default = number of members in household). Include HouseholdCode as human-readable text both in the Household ID line and directly under the barcode.
    7. **Menu:** Reorder top-level menu items (including Switch Role) alphabetically (e.g. Admin, Appointments, Forms, Reports, Switch Role). No main "ID Cards" menu.
    8. **Household Profile ID Cards tab:** New "ID Cards" tab with "Reprint ID Cards" button + number-of-cards selector (default = number of members). Print uses the standard app print flow: attempt direct printing; on failure/no printer, generate temp PDF and open in default viewer.
    9. **New Household post-save:** After save, form closes; show a single success dialog: "Household saved successfully. Complete service now?" The dialog includes an inline **Card count** selector (default = number of household members; minimum 1). Buttons: **Print ID Cards** (uses the inline Card count value to generate the ID card PDF and send it to print (Use standard app print flow - see above); does not open any additional dialogs and does not close the success dialog), **Complete Service** (closes dialog, opens Complete Service for the new household), **No Service** (closes dialog).
  - Likely files:
    - Schema/migration (e.g. `Sql.cs`, `DatabaseMigrator.cs`), `HouseholdRepository`, `HouseholdMemberRepository`
    - `HouseholdProfileForm` (read-only HouseholdCode on main tab, ID Cards tab with Reprint ID Cards + number-of-cards selector)
    - `NewHouseholdForm` (post-save success dialog with Print ID Cards, Complete Service, No Service)
    - PDF generation for ID cards (household-only layout)
...
- [ ] Main page search by name or code; Complete Service dialog: member dropdown, household size, clear search after success
  - Impact: High
  - Complexity: Medium
  - Prerequisite: HouseholdCode and MemberCode columns exist in DB. HouseholdCode shown on Household Profile main tab (see "Add simple human-friendly codes and member ID card generation" above). Codes are not shown on the main check-in grid; search-by-code uses the search box input only.
  - Acceptance Criteria:
