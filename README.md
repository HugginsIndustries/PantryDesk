...
- **Monthly Activity Report & Deck-Only Bulk Entry**:
  - **Monthly Activity Report**: One-page landscape PDF (Reports → Monthly Activity Report). Due-by-10th reminder in header. Configurable header (Food Bank, County, Prepared by, Phone) persisted in config. Content includes: total days open and total pounds distributed; **Households Served** (Duplicated / Unduplicated / Total) and a one-line **Total Households (per city)** breakdown (e.g. `Winlock: 76 · Ryderwood: 37 · Vader: 37`, highest to lowest, `·` separator); **Individuals Served** table by age group (Infant, Child, Adult, Senior) with Duplicated / Unduplicated / Total columns; then three demographic lines in the same format — **Race Distribution**, **Veteran Status** (includes derived “Disabled Veteran” for individuals who are both Veteran and Disabled; they are counted only there to avoid double-counting), and **Disability Status**. Export to PDF or print. Same statistics definition as the Statistics Dashboard (see below).
...
- **QuestPDF** (MIT License) - Used for PDF generation (Statistics Dashboard and Monthly Activity Report). See <https://www.questpdf.com/> for details.
