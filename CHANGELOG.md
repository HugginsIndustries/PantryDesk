...
- **Forms (PDF): Registration and Deck Sign In** (COMPLETED: 2026-02-13, TODO: Forms — Registration and Deck Sign In)
  - "Forms" menu as first menu item (left of Reports) with subitems "Registration" and "Deck Sign In". Shared `BlankFormsForm` dialog: form type label, Export PDF and Print (SaveFileDialog; temp file + shell print verb with fallback to viewer). Export/temp filenames: RegistrationForm.pdf, DeckSignInForm.pdf.
  - **Registration form:** One page, portrait, Letter. Title "Winlock-Vader Food Bank Registration & Shopper Designation Form"; bold subtitle "Information below MUST MATCH your proof of residency information." Contact Info: Street Address; City, State, Zip; Phone #, Email (labels above lines). Household Members: four numbered bold notes (primary/first listed, name+birthday required, optional demographics, race options); table with borders: Name (please print first and last), Birthday (mm/dd/yyyy), Race (see above), Veteran, Disabled (10 rows, header height 30, centered headers, no footer).
  - **Deck Sign In form:** One page, landscape, Letter. Title "Winlock-Vader Food Bank Deck Sign In" (18pt); table with borders: Name (please print first and last), Household Size, Infants (0-2), Children (2-18), Adults (18-55), Seniors (55+), Comment (20 rows, 22pt height); centered column headers; no footer.
